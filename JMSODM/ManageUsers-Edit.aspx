...
<script language="VB" runat="server">
    ...
    Public Sub loadUserDD()
        ...
        'Dim strSQL As String = "Select UserID From tbl_usr_Logins WHERE  (([disabled] IS NULL) OR ([disabled]<>1)) and (NOT (Role_ID = 'Public')) AND (NOT (Role_ID LIKE 'R_Admin_%') Order By UserID"
        Dim strSQL As String = "SELECT distinct [UserID] FROM v_w_ClientUsers WHERE (([disabled] IS NULL) OR ([disabled]<>1)) ORDER BY [UserID]"
    ...
    Public Sub ButtonResetPassword_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnReset.Click
        ...
        Dim dbKey As String = System.Configuration.ConfigurationManager.AppSettings("dbKey")
        Dim strSQL As String = "Select Email From v_w_ClientUsers Where UserID=@UserID"
    ...
    Private Sub LoadUser(ByVal strUID As String)
        ...
        Dim params As SqlParameter() = {
            New SqlParameter("@UserID", strUID)
            }
        Try
            myReader = CommonUtilsv2.GetDataReader(dbKey, strSQL, CommandType.Text, params)
            While myReader.Read
    ...
    Sub RefreshUserRolesList()
        ...
        Dim UsereMail As String = tbEmailEdit.Text.Trim()
        ...
        'Else
        strSQL = "SELECT distinct ur.[Role_ID] , r.RoleName FROM tbl_ROLES_UserRoles ur INNER  JOIN tbl_ROLES_Roles r ON ur.Role_ID =r.Role_ID  WHERE ur.User_ID = @UserID and (NOT (ur.Role_ID = 'Public')) AND (NOT (ur.Role_ID LIKE 'R_Admin_%')) "
        'End If
        LoadDropDownValues(strSQL, "RoleName", "Role_ID", Me.lbxAssignedToValues, strSelected)
        ...
        'Else
        strSQL = "SELECT distinct [Role_ID], RoleName FROM v_w_ClientUserRoles where [Role_ID] NOT IN (select [Role_ID] from [dbo].[tbl_ROLES_UserRoles] where [User_ID] = @UserID) ORDER by [Role_ID]"
        'End If
        LoadDropDownValues(strSQL, "RoleName", "Role_ID", Me.lbxToValues, strSelected)
                ...
                Try
                    For i As Integer = 0 To lbxToValues.Items.Count - 1
                        ...
                        If lbxToValues.Items(i).Selected = True Then
                            ...
                            AddRoleToUser(ddlUserID.SelectedValue, lbxToValues.Items(i).Value)
                        End If
                    Next i
    ...
    Sub UserDropDownLists()
        ...
        '    If (UsereMail.Contains("@jmsassoc.com")) Then
        '        strSQL = "SELECT distinct [UserID] FROM [dbo].[tbl_usr_Logins] ORDER BY [UserID]"
        '    Else
        '        strSQL = "SELECT distinct [UserID] FROM v_w_ClientUsers ORDER BY [UserID]"
        '    End If
        'Else
        strSQL = "SELECT distinct [UserID] FROM v_w_ClientUsers ORDER BY [UserID]"
        'End If
